...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="oars-package-smoke-") as directory:
        env = dict(os.environ, OARS_DATA_DIR=directory + "/data", NATIVE_SDK_LOG_DIR=directory + "/logs")
        env.pop("NATIVE_SDK_FRONTEND_URL", None)
        env.pop("NATIVE_SDK_LOG_FORMAT", None)
        with open(directory + "/launch.log", "w+") as log:
            ...
            try:
                ...
                log.seek(0)
                if 'name="webview.load"' not in log.read():
                    raise RuntimeError("App did not report loading its packaged webview")
                print("Packaged app loaded its webview, created an app window, and remained running.")
            finally:
                if app.poll() is None:
                    app.terminate()
                    try:
